...
const {gridPaths,canMove,rolesFor,flightObjects,targetFor,cargo}=await import('data:text/javascript;base64,'+Buffer.from(source).toString('base64'));
...
for(const n of [6,8,12,16,24,30,36]){const players=new Set();for(let r=0;r<12;r++)for(const id of rolesFor(r,n))players.add(id);assert.equal(players.size,n,'Every child must get a role')}
